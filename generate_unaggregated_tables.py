...
MIDRULE_NOTE = " Models above the midrule are existing/pretrained checkpoints; below are new models from this work."

# Table configurations
TABLE_CONFIGS = {
    "aggregate": {
        "output_file": "latex_arxiv/unaggregated_aggregate.tex",
        "caption": "Individual dataset results across all bioacoustic benchmarks (best per metric in bold). "
    ...
    "beans": {
        ...
        "caption": "BEANS benchmark results: Classification and Detection tasks (best per metric in bold). "
    ...
    "birdset": {
        ...
        "caption": "BirdSet benchmark results: Multi-label bird detection tasks (best per metric in bold). "
    ...
    "complex_tasks": {
        ...
        "caption": "Complex bioacoustic tasks: Individual ID and Vocal Repertoire analysis (best per metric in bold). "
...
def generate_table(df, table_config):
    ...
    if birdset_cols:
        ...
        if len(birdnet_rows) > 0:
            ...
            print(
                f"  Excluding BirdNet results for BirdSet datasets: {list(birdnet_rows)} on {len(birdset_cols)} columns"
            )
        else:
