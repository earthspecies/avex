...
Creates four tables:
1. Aggregate table - all results across benchmarks (excluding clustering for BEANS detection/BirdSet)
...
UNAGG_METRIC_CAPTIONS = {
    'aggregate': (
        "We report ROC AUC for retrieval as R-AUC; probe accuracy for BEANS Classification and Individual ID; "
        "mean-average precision for probe on BEANS Detection and BirdSet; and clustering as NMI where applicable "
        "(BEANS Classification, Vocal Repertoire). "
        "$^{\\dagger}$BirdNet results on BirdSet are excluded following the authors \\citep{rauchbirdset}."
    ),
    'beans': (
        "We report ROC AUC for retrieval as R-AUC; probe accuracy for BEANS Classification and "
        "mean-average precision for BEANS Detection. Clustering is reported as NMI for BEANS Classification only."
    ),
    'birdset': (
        "We report ROC AUC for retrieval as R-AUC and mean-average precision for probe. "
        "No clustering metrics are reported. "
        "$^{\\dagger}$BirdNet results are excluded following the authors \\citep{rauchbirdset}."
    ),
    'complex_tasks': (
...
MIDRULE_NOTE = (
    " Models above the midrule are existing/pretrained checkpoints; below are new models from this work."
)
...
TABLE_CONFIGS = {
    'aggregate': {
        ...
        'caption': 'Individual dataset results across all bioacoustic benchmarks (best per metric in bold). ' + UNAGG_METRIC_CAPTIONS['aggregate'] + MIDRULE_NOTE,
        'label': 'tab:unaggregated-aggregate',
        'datasets': [],  # Will be populated with all datasets
        'separators': []  # No separators for aggregate table (models as rows, not datasets)
    },
    'beans': {
        'output_file': 'latex_arxiv/unaggregated_beans.tex',
        'caption': 'BEANS benchmark results: Classification and Detection tasks (best per metric in bold). ' + UNAGG_METRIC_CAPTIONS['beans'] + MIDRULE_NOTE,
        'label': 'tab:unaggregated-beans',
        'datasets': BENCHMARK_DATASETS["BEANS Classification"] + BENCHMARK_DATASETS["BEANS Detection"],
    ...
    'beans_classification': {
        ...
        'caption': 'BEANS Classification datasets only (best per metric in bold). ' +
                   'We report R-AUC for retrieval; probe accuracy; clustering reported as NMI. ' + MIDRULE_NOTE,
    ...
    'beans_detection': {
        ...
        'caption': 'BEANS Detection datasets only (best per metric in bold). ' +
                   'We report R-AUC for retrieval and mean-average precision for probe. ' + MIDRULE_NOTE,
    ...
    'birdset': {
        ...
        'caption': 'BirdSet benchmark results: Multi-label bird detection tasks (best per metric in bold). ' + UNAGG_METRIC_CAPTIONS['birdset'] + MIDRULE_NOTE,
    ...
    'complex_tasks': {
        ...
        'caption': 'Complex bioacoustic tasks: Individual ID and Vocal Repertoire analysis (best per metric in bold). ' + UNAGG_METRIC_CAPTIONS['complex_tasks'] + MIDRULE_NOTE,
...
def load_excel_data():
    ...
    mask = ~raw.index.str.contains('|'.join(EXCLUDE_MODELS))
    filtered = raw[mask]
    
    print(f"📋 Filtered data shape: {filtered.shape}")
    return filtered

def get_dataset_columns(df, datasets):
        ...
        for metric in metrics:
            ...
            if dataset in BENCHMARK_DATASETS["Individual ID"] and metric == "R-auc":
                ...
                if cross_key in df.columns:
                    ...
                    dataset_cols.append(f"R-auc*")  # Mark as cross-validation
...
def get_benchmark_group(dataset):
...
def build_latex_table(fmt_df, dataset_info, separators_after, caption, label):
    ...
    header2 = ['\\textbf{Model}']
    for dataset_name, _, metrics in dataset_info:
        for m in metrics:
    ...
    # Track current benchmark group for separators
    current_group = None
...
def generate_table(df, table_config):
    ...
    if birdset_cols:
        ...
        birdnet_rows = selected_df.index[selected_df.index.str.contains("BirdNet", case=False, na=False)]
        if len(birdnet_rows) > 0:
            for col in birdset_cols:
                selected_df.loc[birdnet_rows, col] = np.nan
            print(f"  Excluding BirdNet results for BirdSet datasets: {list(birdnet_rows)} on {len(birdset_cols)} columns")
        else:
            print(f"  BirdNet not found in index for BirdSet exclusion")
    ...
    existing_priority = [
        'Perch', 'SurfPerch', 'BirdNet', 'BEATS (SFT)', 'BEATS (pretrained)',
        'EAT-base (pretrained)', 'EAT-base (SFT)', 'Bird-AVES-biox-base', 'BEATS-NatureLM-audio'
    ]
