...
UNAGG_METRIC_CAPTIONS = {
    "aggregate": (
        "We report ROC AUC for retrieval as R-AUC; probe accuracy for BEANS "
        "Classification and Individual ID; mean-average precision for probe on "
        "BEANS Detection and BirdSet; and clustering as NMI where applicable "
        "(BEANS Classification, Vocal Repertoire). "
        "$^{\\dagger}$BirdNet results on BirdSet are excluded following the authors "
        "\\citep{rauchbirdset}."
    ),
    "beans": (
        "We report ROC AUC for retrieval as R-AUC; probe accuracy for BEANS "
        "Classification and mean-average precision for BEANS Detection. "
        "Clustering is reported as NMI for BEANS Classification only."
    ),
    "birdset": (
        "We report ROC AUC for retrieval as R-AUC and mean-average precision "
        "for probe. No clustering metrics are reported. "
        "$^{\\dagger}$BirdNet results are excluded following the authors "
        "\\citep{rauchbirdset}."
    ),
    "complex_tasks": (
        "We report ROC AUC for retrieval as R-AUC. Individual ID probe is "
        "accuracy; Vocal Repertoire reports both R-AUC and NMI."
    ),
}
...
TABLE_CONFIGS = {
    ...
    "beans_classification": {
        ...
        "caption": (
            "BEANS Classification datasets only (best per metric in bold). "
            "We report R-AUC for retrieval; probe accuracy; clustering "
            "reported as NMI. " + MIDRULE_NOTE
        ),
    ...
    "beans_detection": {
        ...
        "caption": (
            "BEANS Detection datasets only (best per metric in bold). "
...
def load_excel_data() -> pd.DataFrame:
    ...
    -------
    pd.DataFrame
        Filtered dataframe containing the raw model results.
    """
...
def get_dataset_columns(
    ...
    """Get available metric columns for the requested datasets.

    Parameters
    ----------
    df : pd.DataFrame
        Source dataframe containing metrics.
    datasets : list[str]
    ...
    -------
    tuple[list[str], list[tuple[str, int, list[str]]]]
        Tuple of selected column names and dataset metadata
        ``(dataset_name, column_count, display_metrics)``.
    """
    available_columns = []
    dataset_info = []
...
def apply_bold_formatting(df: pd.DataFrame) -> pd.DataFrame:
    """Apply bold formatting to maximum values with BirdNet special casing.
    ...
    -------
    pd.DataFrame
        Formatted dataframe with bolded maxima and "N/A" markers.
    """
...
def get_benchmark_group(dataset: str) -> str:
    ...
    -------
    str
        Benchmark group name if found, otherwise ``"Unknown"``.
    """
...
def generate_table(df: pd.DataFrame, table_config: dict) -> str | None:
    """Generate a single table based on configuration.

    Parameters
    ----------
    df : pd.DataFrame
        Source dataframe with metrics.
    table_config : dict
        Configuration for the table to generate.

    Returns
    -------
    str | None
        Generated LaTeX content, or ``None`` if no columns were available.
    """
