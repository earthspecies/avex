...
def extract_groups_and_metrics() -> tuple[dict[str, list[str]], dict[str, list[str]]]:
    """Extract metrics from ``visualize_excel.py``.

    Returns
    -------
    tuple[dict[str, list[str]], dict[str, list[str]]]
        Tuple containing the corrected group datasets and the metrics
        dictionary parsed from ``visualize_excel.py``.
    """
...
def validate_dataset_coverage(
    ...
    """Validate that all expected datasets exist in the dataframe.

    Parameters
    ----------
    df : pd.DataFrame
        Dataframe containing all model metrics.
    group_datasets : dict[str, list[str]]
        Mapping of benchmark group to a list of dataset identifiers.
    metrics : dict[str, list[str]]
        Mapping of benchmark group to the metrics expected for that group.

    Returns
    -------
    tuple[list[tuple[str, str]], list[tuple[str, str]]]
        A tuple containing lists of missing and found dataset tuples respectively.
    """
...
def compute_group_averages_with_prioritization(
    df: pd.DataFrame,
    group_datasets: dict[str, list[str]],
    metrics: dict[str, list[str]] | list[str],
) -> pd.DataFrame:
    """Compute per-model averages with benchmark-specific rules.

    Parameters
    ----------
    df : pd.DataFrame
        Input dataframe with model-level metrics.
    group_datasets : dict[str, list[str]]
        Mapping of benchmark group to datasets.
    metrics : dict[str, list[str]] | list[str]
        Mapping of benchmark group to metrics (or a flat list of metrics).

    Returns
    -------
    pd.DataFrame
        Dataframe containing computed averages per benchmark group.
    """
                ...
                if keys:
                    ...
                    if group_name == "BirdSet":
                        ...
                        if len(birdnet_rows) > 0:
                            ...
                            print(
                                "  🚫 Excluding BirdNet results for BirdSet datasets: "
                                f"{birdnet_exclusion}"
                            )
                        else:
...
def load_and_process_data() -> pd.DataFrame:
    """Load Excel data and compute sanitized group averages.

    Returns
    -------
    pd.DataFrame
        Processed dataframe ready for LaTeX table generation.
    """
...
def build_column_structure(
    ...
    include_groups : list[str] | None, optional
        Optional subset of groups to include.

    Returns
    -------
    tuple[list[str], list[tuple[str, int, list[str]]]]
        Tuple of selected columns and group metadata.
    """
    cols = []
    group_info = []
...
def _determine_model_type(raw_name: str) -> str:
    ...
    -------
    str
        Training tag classification for the model.
    """
    name = raw_name or ""
    lower = name.lower()
...
def _rename_model_for_display(raw_name: str) -> str:
    ...
    -------
    str
        Display name with overrides applied.
    """
    rename_map = {
...
def decorate_model_names_for_table(
    ...
    *,
    ...
    """Decorate model names with training tags and dagger markers.

    Returns
    -------
    tuple[pd.DataFrame, dict]
        Tuple containing the decorated dataframe and a mapping from raw names
        to decorated labels.
    """
    display_names = []
    mapping = {}
...
def reorder_models_with_separator(df: pd.DataFrame) -> tuple[pd.DataFrame, str]:
    ...
    tuple[pd.DataFrame, str]
        Reordered dataframe and the raw name of the last existing model.
    """
...
def apply_bold_formatting(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Includes special N/A handling for BirdNet on BirdSet benchmarks.
    ...
    -------
    pd.DataFrame
        Formatted dataframe with bolded maxima.
    """
...
def build_latex_table(
    ...
    fmt_df : pd.DataFrame
        Formatted dataframe.
    group_info : list[tuple[str, int, list[str]]]
        Group metadata tuples.
    separators_after : list[str] | None, optional
        List of model display names after which to insert separators.
    ...
    -------
    tuple[str, str]
        Rendered LaTeX content and the column format string.
    """

    caption = caption or TABLE_CAPTION
    label = label or TABLE_LABEL
    separators_after = separators_after or []
...
def generate_sub_table(df: pd.DataFrame, table_config: dict) -> str | None:
    ...
    table_config : dict
        Sub-table configuration dictionary.

    Returns
    -------
    str | None
        Generated LaTeX content, or ``None`` when no models are available.
    """
        ...
        if model == "---SEPARATOR---":
            ...
            if model in df.index:
                ...
                if len(matches) > 0:
                    ...
                    try:
                        matches = df.index[
                            df.index.str.contains(escaped_model, regex=True, case=False)
                        ]
                        if len(matches) > 0:
                            available_models.append(matches[0])
                        else:
...
def main() -> None:
    ...
    latex_content, colfmt = build_latex_table(
        formatted_df, group_info, separators_after=main_separators
    )
