...
METRIC_CAPTION_SUFFIX = (
    ...
    r"mean-average precision for probe on BEANS Detection and BirdSet. We report the mean of each metric over datasets per benchmark. $^{\dagger}$BirdNet results on BirdSet are excluded following the authors \citep{rauchbirdset} due to data leakage"
)
...
SUB_TABLES = {
    "efficientnet_analysis": {
        ...
        "caption": "Comparison of EfficientNet models trained on mixes of AudioSet and large-scale bioacoustic data. "
        + METRIC_CAPTION_SUFFIX,
        ...
        for metric in metrics:
            if group_name == "Individual ID" and metric == "R-auc":
                ...
                if prioritized_values:
                    ...
                    if total_nans > 0:
                        print(
                            f"  ⚠️  Found {total_nans}/{total_cells} empty cells, excluding from average"
                        )
                    ...
                    print(
                        f"  Individual ID R-auc: Combined {len(prioritized_values)} datasets"
                    )
                ...
                if missing_keys:
                    print(
                        f"  ❌ Missing columns for {group_name} {metric}: {missing_keys}"
                    )
                ...
                if keys:
                    ...
                    if group_name == "BirdSet":
                        ...
                        if len(birdnet_rows) > 0:
                            ...
                            print(
                                f"  🚫 Excluding BirdNet results for BirdSet datasets: {birdnet_exclusion}"
                            )
                        else:
                            print(
                    ...
                    result[f"{group_name}_{metric}"] = avg
                    print(
                        f"  ✅ {group_name} {metric}: Found {len(keys)}/{len(datasets)} datasets - {keys}"
                    )
                else:
...
def load_and_process_data():
    ...
    print(
        f"🔧 Corrected BEANS Classification: {CORRECTED_GROUP_DATASETS['BEANS Classification']}"
    )
...
def build_latex_table(
    fmt_df, group_info, separators_after=None, caption=None, label=None
):
            ...
            if model in df.index:
                ...
                if len(matches) > 0:
                    ...
                    try:
                        ...
                        print(
                            f"Warning: Model '{model}' not found in data (regex error: {e})"
                        )
