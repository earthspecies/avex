"""Tests for WeightedMLPProbe."""

import pytest
import torch

from representation_learning.models.base_model import ModelBase
from representation_learning.models.probes.weighted_mlp_probe import WeightedMLPProbe
...
class TestWeightedMLPProbe:
    """Test cases for WeightedMLPProbe."""

    def test_feature_mode_with_input_dim(self) -> None:
        """Test WeightedMLPProbe in feature mode with provided input_dim."""
        input_dim = 512
        num_classes = 10
        batch_size = 4
        hidden_dims = [256, 128]

        probe = WeightedMLPProbe(
            base_model=None,
            layers=[],
    ...
    def test_feature_mode_with_base_model(self) -> None:
        """Test WeightedMLPProbe in feature mode with base_model."""
        embedding_dims = [256]
        num_classes = 5
        batch_size = 2
        hidden_dims = [128, 64]
    ...
    def test_single_tensor_case(self) -> None:
        """Test WeightedMLPProbe with single tensor embeddings."""
        embedding_dims = [256]
        num_classes = 8
        batch_size = 3
        hidden_dims = [128]
    ...
    def test_list_embeddings_same_dimensions(self) -> None:
        """Test WeightedMLPProbe with list of embeddings having same dimensions."""
        ...
        assert debug_info["probe_type"] == "weighted_mlp"
    ...
    def test_list_embeddings_different_dimensions_raises_error(self) -> None:
        """Test that WeightedMLPProbe raises error for different embedding
        dimensions."""
    ...
    def test_feature_mode_without_input_dim_raises_error(self) -> None:
        """Test that WeightedMLPProbe raises error in feature mode without input_dim."""
        with pytest.raises(
    ...
    def test_default_hidden_dims(self) -> None:
        """Test WeightedMLPProbe with default hidden_dims."""
        input_dim = 128
        num_classes = 4
        batch_size = 2
        ...
        probe = WeightedMLPProbe(
            ...
            feature_mode=True,
            input_dim=input_dim,
            # hidden_dims not specified, should use default [512, 256]
        )

        assert probe.hidden_dims == [512, 256]
    ...
    def test_dropout_rate(self) -> None:
        """Test WeightedMLPProbe with different dropout rates."""
        input_dim = 64
        num_classes = 3
        batch_size = 2
        hidden_dims = [32]
    ...
    def test_no_dropout(self) -> None:
        """Test WeightedMLPProbe without dropout."""
        input_dim = 64
        num_classes = 3
        batch_size = 2
        hidden_dims = [32]
    ...
    def test_activation_functions(self) -> None:
        """Test WeightedMLPProbe with different activation functions."""
        input_dim = 32
        num_classes = 2
        batch_size = 2
        hidden_dims = [16]
        ...
        for activation in ["relu", "gelu", "tanh"]:
            ...
            assert probe.activation == activation
    ...
    def test_unsupported_activation_raises_error(self) -> None:
        """Test that WeightedMLPProbe raises error for unsupported activation."""
        input_dim = 32
        num_classes = 2
        hidden_dims = [16]
    ...
    def test_use_positional_encoding(self) -> None:
        """Test WeightedMLPProbe with positional encoding."""
        input_dim = 64
        num_classes = 3
        batch_size = 2
        hidden_dims = [32]
        ...
        assert probe.use_positional_encoding is True
    ...
    def test_projection_dim_parameter(self) -> None:
        """Test WeightedMLPProbe with projection_dim parameter."""
        input_dim = 128
        num_classes = 4
        batch_size = 2
        hidden_dims = [64]
    ...
    def test_freeze_backbone(self) -> None:
        """Test WeightedMLPProbe with frozen backbone."""
        embedding_dims = [256]
        num_classes = 3
        batch_size = 2
        hidden_dims = [128]
    ...
    def test_dict_input(self) -> None:
        """Test WeightedMLPProbe with dictionary input."""
        input_dim = 64
        num_classes = 2
        batch_size = 2
        hidden_dims = [32]
    ...
    def test_debug_info(self) -> None:
        ...
        assert debug_info["hidden_dims"] == hidden_dims
        assert debug_info["activation"] == "relu"  # default
    ...
    def test_2d_embedding_requirement(self) -> None:
        """Test that WeightedMLPProbe requires 2D embeddings."""
        ...
        with pytest.raises(ValueError, match="MLP probe expects 2D embeddings"):
            WeightedMLPProbe(
                base_model=base_model,
                layers=["layer1"],
                num_classes=5,
    ...
    def test_target_length_parameter(self) -> None:
        """Test WeightedMLPProbe with target_length parameter."""
        input_dim = 96
        num_classes = 3
        batch_size = 2
        target_length = 2000
