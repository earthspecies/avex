...
# -----------------------------------------------------------------------------
# Helper: frame sequence -> (2, N) array of [onset; offset] times (seconds)
# -----------------------------------------------------------------------------

def _frames_to_events(frames: np.ndarray, fps: float) -> np.ndarray:
    """Convert a 1-D binary frame sequence into onset/offset event pairs.

    Parameters
    ----------
    frames : np.ndarray, shape (T,)
        Binary (0/1) frame activity sequence.
    fps : float
    ...
    -------
    np.ndarray
        Array of shape ``(2, N)`` where row 0 contains onset times and row 1
        contains offset times (in *seconds*).
    """
    if frames.ndim != 1:
        raise ValueError("frames must be 1-D")

    # Pad with zero at both ends to detect edges cleanly
    padded = np.pad(frames.astype(bool).astype(int), (1, 1), constant_values=0)
    changes = np.diff(padded)
    onsets = np.where(changes == 1)[0]
    offsets = np.where(changes == -1)[0]

    if len(onsets) != len(offsets):
        raise RuntimeError("Mismatch between detected onsets and offsets")

    return np.vstack([onsets / fps, offsets / fps])
