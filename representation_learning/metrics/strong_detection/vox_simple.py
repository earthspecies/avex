def voxaboxen_metrics(ref_events, pred_events, iou_thr=(0.5, 0.8)):
    ...
    # ---------- F1 ----------
    ev = sed_eval.sound_event.EventBasedMetrics(labels, t_collar=0.0,
                                                percentage_of_length=0.0,
                                                event_matching_type='optimal')
    for f in el.unique_files(ref_list):
        ev.evaluate(el.filter_event_list(ref_list, f),
                    el.filter_event_list(pred_list, f))
