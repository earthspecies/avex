...
class StrongDetectionF1Metric:
    def __init__(self, iou: float = 0.5, class_threshold: float = 0.5,
                 label_set: list = None, unknown_label: str = None, view: str = None, label_mapping: dict = None, base_path_prediction="", base_path_reference="", merge_labels: bool = False):
        """
        Parameters:
    ...
    def __call__(self, reference_fp: Path, prediction_fp: Path, thresholding: bool = False, default_duration: float = 20) -> float:
        """
        ...
        for _, row in merged.iterrows():
            ...
            clip = Clip(label_set=self.label_set, unknown_label=self.unknown_label, merge_labels=self.merge_labels)
            ...
            if selection_ref is None or selection_pred is None:
                print(f"Skipping clip {row.get('fn')} because one of the selection table paths is missing.")
                continue

            clip.load_annotations(os.path.join(self.base_path_reference, str(selection_ref)), view=self.view, label_mapping=self.label_mapping)
            clip.load_predictions(os.path.join(self.base_path_prediction, str(selection_pred)), view=self.view, label_mapping=self.label_mapping)
