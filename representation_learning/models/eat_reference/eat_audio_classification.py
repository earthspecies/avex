...
class MaeImageClassificationConfig(FairseqDataclass):
    ...
    audio_mae: bool = field(
        default=False,
        metadata={"help": "if true, the task is to realize audio classification"},
    )
    esc50_eval: bool = field(
        default=False,
        metadata={"help": "if true, the task is to finetune model on esc50 dataset"},
    )
    ...
    target_length: int = field(
        default=1024,
        metadata={"help": "This setting will pad the input sequence will zeros."},
    )
...
class MaeImageClassificationModel(BaseFairseqModel):
    def __init__(self, cfg: MaeImageClassificationConfig):
        ...
        if "modalities" in pretrained_args.model:
            ...
            with open_dict(pretrained_args):
                ...
                pretrained_args.model["modalities"]["image"]["target_length"] = (
                    cfg.target_length
                )
        else:
            # not d2v multi
    ...
    def forward(
        ...
        if not self.audio_mae or (
            self.audio_mae and (self.esc50_eval or self.spcv2_eval)
        ):
