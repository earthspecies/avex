...
class Data2VecMultiConfig(FairseqDataclass):
    ...
    utterance_level: bool = field(
        default=False,
        metadata={
            "help": "if true, we will add utterance-level loss to the total loss"
        },
    )
    init_center_token_zero: bool = field(
        default=False,
        metadata={
            "help": "if true, we will initialize the centor token with zero vertors"
        },
    )
    center_exp: float = field(
        default=0.9,
        metadata={
            "help": "this value control the exponent decay of center value's coefficient"
        },
    )
    softmax_temperature_student: float = field(
        default=0.1,
        metadata={
            "help": "this value control the temperature of softmax function of student output in the dino loss"
        },
    )
    softmax_temperature_teacher: float = field(
        default=0.05,
        metadata={
            "help": "this value control the temperature of softmax function in teacher output the dino loss"
        },
    )
...
class Data2VecMultiModel(BaseFairseqModel):
    ...
    def make_target_model(self):
        ...
        if self.cfg.ema_encoder_only:
            ...
            for p_s, p_t in zip(
                self.blocks.parameters(), model_copy.parameters(), strict=False
            ):
                p_t.data.copy_(p_s.data)
        else:
            for p_s, p_t in zip(
                self.parameters(), model_copy.parameters(), strict=False
            ):
                p_t.data.copy_(p_s.data)
    ...
    def forward(
        ...
        if self.cfg.cls_loss > 0 and self.utterance_level:
            ...
            cls_target = orig_targets.mean(dim=1)
            if self.cfg.clone_batch > 1:
                cls_target = cls_target.repeat_interleave(
            ...
            self.center = self.center_exp * self.center + (1 - self.center_exp) * (
                cls_target.mean(dim=0)
            )
