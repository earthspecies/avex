"""Weighted Attention probe for representation learning evaluation."""
...
class WeightedAttentionProbe(torch.nn.Module):
    """
    Weighted Attention probe for sequence-based representation learning evaluation.
    ...
    Args:
        ...
        # Note: Embedding projectors are automatically applied when needed.
        # Target dimensions are determined from the maximum sequence length
        # and feature dimensions across all embeddings.
    """
    ...
    def __init__(
        ...
        if self.feature_mode:
            ...
            if input_dim is not None:
                ...
                with torch.no_grad():
                    ...
                    if dummy_embeddings.dim() == 3:
                        # For attention probes, we expect 3D embeddings
            ...
            with torch.no_grad():
                ...
                if isinstance(dummy_embeddings, list):
                    ...
                    logger.info(f"Attention dim: {self.attention_dim}")
                    ...
                    if needs_projection:
                        ...
                        logger.info(
                            f"Attention internal dimension: {self.attention_dim}"
                        )
                        logger.info(
                            "Note: attention_dim is internal to MultiheadAttention, "
                            "not input feature dim"
                        )
                    else:
                    ...
                    # Single tensor case
                    logger.debug(
                        f"Attention probe: Single tensor case - dummy_embeddings type: "
                    ...
                    if dummy_embeddings.dim() == 2:
                        ...
                        inferred_dim = dummy_embeddings.shape[-1]
                        logger.debug(
                            f"Attention probe: Using 3D tensor with "
                            f"inferred_dim: {inferred_dim}"
                        )
                    elif dummy_embeddings.dim() == 4:
                        # 4D: (batch, channels, height, width) -> reshape to 3D
                        ...
                        inferred_dim = feature_dim
                        logger.debug(
                            f"Attention probe: Reshaped 4D tensor from "
                            f"{dummy_embeddings.shape} "
                            f"to 3D with inferred_dim: {inferred_dim}"
                        )
                    else:
                        logger.error(
                            f"Attention probe: Expected 2D, 3D, or 4D embeddings "
                            f"but got shape "
        ...
        # Note: embedding_projectors are only created for list cases above
        # Single tensor cases don't use projectors
        ...
        if use_positional_encoding:
            self.pos_encoding = SinusoidalPositionalEncoding(
                inferred_dim, max_sequence_length or 1000
            )
        else:
        ...
        logger.info(
            f"WeightedAttentionProbe initialized with final parameters: "
    ...
    def forward(
        ...
        """Forward pass through the weighted attention probe.
        ...
        if self.feature_mode:
            ...
            logger.debug(
                f"Attention probe forward: Calling base_model.extract_embeddings "
        ...
        if not isinstance(embeddings, list):
            if embeddings.dim() == 2:
                # 2D: (batch, features) -> add feature dimension of size 1
                embeddings = embeddings.unsqueeze(2)  # (batch, features, 1)
