...
class _BaseProbe(ABC, nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        inferred_dim, num_embeddings = self._setup_projections_and_infer_dim(
            input_dim=input_dim
        )
    ...
    def _extract_dummy_embeddings_from_model(self) -> TensorOrList:
        ...
        with torch.no_grad():
            ...
            return self.base_model.extract_embeddings(
    ...
    def _setup_projections_and_infer_dim(
        ...
        *,
        ...
        if self.feature_mode and input_dim is not None:
            dummy_embeddings: TensorOrList = self._get_dummy_embeddings_from_input_dim(
                input_dim
            )
    ...
    def get_learned_weights_table(self) -> str:
        ...
        """
        if (
            not hasattr(self, "layer_weights")
            or getattr(self, "layer_weights", None) is None
        ):
            return (
        ...
        for i, (r, n) in enumerate(zip(raw, norm, strict=False)):
            ...
            pct = n * 100
            weights_table.append(
                f"{layer_name:<15} {r:<12.4f} {n:<12.4f} {pct:<12.2f}%"
            )
        weights_table.append("-" * 50)
...
class BaseProbe2D(_BaseProbe):
    ...
    def _infer_single_tensor_dim(self, emb: torch.Tensor) -> int:
        ...
        raise ValueError(
            f"Linear probe expects 2D, 3D or 4D embeddings, got {emb.shape}"
        )
    ...
    def _combine_or_reshape_embeddings(self, embeddings: TensorOrList) -> torch.Tensor:
        if isinstance(embeddings, list):
            ...
            for emb, projector in zip(
                embeddings, self.embedding_projectors, strict=False
            ):
                if emb.dim() == 4:
                    ...
                    raise ValueError(
                        f"Unsupported embedding dim {emb.dim()} for 2D probe"
                    )
        ...
        if embeddings.dim() == 2:
            return embeddings
        raise ValueError(
            f"Linear probe expects 2D, 3D or 4D embeddings, got {embeddings.shape}"
        )
...
class BaseProbe3D(_BaseProbe):
    ...
    def _analyze_and_create_projectors(self, embeddings: List[torch.Tensor]) -> int:
        ...
        target_seq = (
            most_seq if seq_count > len(info) / 2 else max(s for _, s, _ in info)
        )
        target_feat = (
            most_feat if feat_count > len(info) / 2 else max(f for _, _, f in info)
        )
    ...
    def _infer_single_tensor_dim(self, emb: torch.Tensor) -> int:
        ...
        if emb.dim() == 4:
            return emb.shape[1] * emb.shape[2]
        raise ValueError(
            f"Attention probe expects 2D, 3D or 4D embeddings, got {emb.shape}"
        )
