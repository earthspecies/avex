...
class _BaseProbe(ABC, nn.Module):
    ...
    @abstractmethod
    def forward(self, x: Union[torch.Tensor, dict], padding_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        *,
        ...
        if self.feature_mode and input_dim is not None:
            ...
            if self.feature_mode and self.base_model is None and input_dim is None:
                raise ValueError("input_dim must be provided when feature_mode=True and base_model is None")
    ...
    def _get_embeddings(self, x: Union[torch.Tensor, dict], padding_mask: Optional[torch.Tensor]) -> TensorOrList:
    ...
    def get_learned_weights_table(self) -> str:
        ...
        weights_table.append(f"{'Layer':<15} {'Raw Weight':<12} {'Normalized':<12} {'Percentage':<12}")
        weights_table.append("-" * 50)
