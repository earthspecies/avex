"""Weighted LSTM probe for representation learning evaluation."""
...
class WeightedLSTMProbe(torch.nn.Module):
    """
    Weighted LSTM probe for sequence-based representation learning evaluation.

    The probe extracts embeddings from specified layers of a **base_model** and
    processes them through a single LSTM for sequence modeling. For multiple
    layer embeddings, it uses learned weights to combine them.
    ...
    def __init__(
                ...
                if isinstance(dummy_embeddings, list):
                    ...
                    for i, emb in enumerate(dummy_embeddings):
                        ...
                        if emb.dim() == 4:
                            ...
                            # channels * height
                            feature_dim = emb.shape[1] * emb.shape[2]
                        elif emb.dim() == 3:
                    ...
                    if needs_projection:
                        ...
                        lstm_true_hidden_size = int(
                            np.maximum(int(max_seq_len / 4), self.lstm_hidden_size)
                        )
                        logger.info(
                            f"Final projected feature dimension: {inferred_dim}"
                        )
                        logger.info(f"LSTM hidden size: {lstm_true_hidden_size}")
                    else:
                    ...
                    # Handle single 4D tensor by reshaping to 3D (same as LSTMProbe)
        ...
        # Create single LSTM for all cases
        self.lstm = nn.LSTM(
        ...
        classifier_input_dim = lstm_true_hidden_size * (2 if bidirectional else 1)
        ...
        logger.info(
            f"WeightedLSTMProbe initialized with final parameters: "
    ...
    def forward(
        ...
        """Forward pass through the weighted LSTM probe.
