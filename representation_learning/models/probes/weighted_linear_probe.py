"""Weighted Linear probe for representation learning evaluation."""
...
class WeightedLinearProbe(torch.nn.Module):
    """
    Weighted Linear probe for representation learning evaluation.

    The probe extracts embeddings from specified layers of a **base_model** and
    processes them through a single linear classifier. For multiple layer
    ...
    def __init__(
        ...
        if self.feature_mode:
            ...
            if input_dim is not None:
                ...
                with torch.no_grad():
                    ...
                    logger.info(
                        f"Input to Linear probe shape: {dummy_embeddings.shape}"
                    )
            ...
            with torch.no_grad():
                ...
                if isinstance(dummy_embeddings, list):
                    ...
                    # Single tensor case
                    logger.debug(
                        f"Linear probe: Single tensor case - dummy_embeddings type: "
        ...
        # Create single linear classifier for all cases
        ...
        logger.info(
            f"WeightedLinearProbe initialized with final parameters: "
    ...
    def forward(
        ...
        """Forward pass through the weighted linear probe.
        ...
        Args:
            ...
            padding_mask: Optional padding mask for the input. Currently not used
                by linear probes but included for compatibility with training loops.
        ...
        if self.feature_mode:
            ...
            logger.debug(
                f"Linear probe forward: Calling base_model.extract_embeddings "
        ...
        if isinstance(embeddings, list):
            ...
            for i, emb in enumerate(embeddings):
                if emb.shape != (batch_size, embedding_dim):
                    raise ValueError(
                        f"Embedding {i} has shape {emb.shape}, "
