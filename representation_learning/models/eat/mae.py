...
# your own versions.
try:
    from .modules import AltAttention as AltAttention2, AltBlock as AltBlock2
except Exception:
...
def _get_2d_sincos_pos_from_grid(embed_dim, grid):
...
def get_2d_sincos_pos_embed_flexible(embed_dim: int, grid_hw: tuple[int, int], cls_token=False):
    ...
    square images.  The implementation simply constructs separate (H, W)
    grids and concatenates their 1-D embeddings.
    """
...
class MaeModel(nn.Module):
    def __init__(self, cfg: MaeConfig):
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(
                1, num_patches + (0 if cfg.no_cls else 1), cfg.embed_dim
            ),
            requires_grad=False,
        )
        ...
        def make_block(drop_path):
            if cfg.w2v_block:
                raise NotImplementedError(
                    "cfg.w2v_block=True requires fairseq's TransformerSentenceEncoderLayer; "
                    "set it to False or add your own layer."
                )
        ...
        self.mask_token = (
            ...
            else nn.Parameter(torch.zeros(1, 1, dec_dim))
        )

        self.decoder_pos_embed = (
            None
            if cfg.no_decoder_pos_embed
            else nn.Parameter(torch.zeros(1, num_patches + 1, dec_dim), requires_grad=False)
        )
    ...
    def patchify(self, imgs: torch.Tensor) -> torch.Tensor:
        ...
        x = torch.einsum("nchpwq->nhwpqc", x).reshape(imgs.size(0), h * w, p * p * self.cfg.in_chans)
        return x
    ...
    def unpatchify(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = torch.einsum("nhwpqc->nchpwq", x).reshape(x.size(0), self.cfg.in_chans, h * p, w * p)
        return x
    ...
    # ------------------------------------------------------------------------- #
    @staticmethod
    def _random_masking(x: torch.Tensor, ratio: float):
        N, L, D = x.shape
        keep = int(L * (1 - ratio))
    ...
    #  Encoder                                                                  #
    # ------------------------------------------------------------------------- #
    def _forward_encoder(self, imgs: torch.Tensor, ratio: float):
    ...
    #  Decoder                                                                  #
    # ------------------------------------------------------------------------- #
    def _forward_decoder(self, x: torch.Tensor, ids_restore: torch.Tensor):
    ...
    def forward(self, imgs: torch.Tensor, predictions_only: bool = False):
