...
def test_save_and_load_evaluation_metadata(
    ...
    assert df["model_spec_name"].iloc[0] == "efficientnet"
    assert df["model_spec_efficientnet_variant"].iloc[0] == "b0"
    assert df["model_spec_audio_config_sample_rate"].iloc[0] == 16000
...
def test_evaluation_metadata_with_training_metadata(
    ...
    """Test saving evaluation metadata with training metadata."""
