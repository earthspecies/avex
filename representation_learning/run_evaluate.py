...
def run_experiment(
    ...
    train_dl, val_dl, test_dl = build_dataloaders(original_run_cfg, dataset_config, device=device)
    logger.info(
    ...
    if experiment_config.layers == "last_layer":
        layer_names = [name for name, module in model.named_modules() if isinstance(module, torch.nn.Linear)]
