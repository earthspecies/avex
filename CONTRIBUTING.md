...
Continuous integration runs tests on **Python 3.10** and **3.13**. Slow official-model output regression tests use profiled expected fingerprints for **torch 2.5.x** vs **torch 2.11.x** (`tests/integration/test_official_models_output_regression.py`); regenerate with `scripts/regenerate_official_model_output_fingerprints.py` if you change the numerical stack.
