...
local['mkdir']['-p', 'benchmark/rfcx/wav']()
local['kaggle']['competitions', 'download', '-p', 'benchmark/rfcx', 'rfcx-species-audio-detection'] & FG
local['unzip']['benchmark/rfcx/rfcx-species-audio-detection.zip', '-d', 'benchmark/rfcx/'] & FG

dest_dir = pathlib.Path('benchmark/rfcx/wav')
for src_file in sorted(pathlib.Path('benchmark/rfcx/train').glob('*.flac')):
...
df = pd.read_csv('benchmark/rfcx/train_tp.csv')
...
with open('benchmark/rfcx/train.jsonl', mode='w') as f:
    for data in df_train:
        print(json.dumps(data), file=f)

with open('benchmark/rfcx/train-low.jsonl', mode='w') as f:
    for data in df_train_low:
        print(json.dumps(data), file=f)

with open('benchmark/rfcx/valid.jsonl', mode='w') as f:
    for data in df_valid:
        print(json.dumps(data), file=f)

with open('benchmark/rfcx/test.jsonl', mode='w') as f:
    for data in df_test:
        print(json.dumps(data), file=f)
