...
local['mkdir']['-p', 'benchmark/dcase/wav']()
local['wget']['https://zenodo.org/record/5412896/files/Development_Set.zip?download=1', '-O', 'benchmark/dcase/Development_set.zip'] & FG
local['unzip']['benchmark/dcase/Development_set.zip', '-d', 'benchmark/dcase/'] & FG

datasets = defaultdict(list)

for wav_path in sorted(Path('benchmark/dcase/Development_Set/').glob('**/*.wav')):
    ...
    target_paths = divide_waveform_to_chunks(
        path=wav_path,
        target_dir='benchmark/dcase/wav/',
...
for split in ['train', 'train-low', 'valid', 'test']:
    with open(f'benchmark/dcase/{split}.jsonl', mode='w') as f:
