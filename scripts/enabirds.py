...
local['mkdir']['-p', 'benchmark/enabirds/wav']()
local['wget']['https://storage.googleapis.com/ml-bioacoustics-datasets/enabirds_wav.zip', '-O', 'benchmark/enabirds/wav_Files.zip'] & FG
local['unzip']['benchmark/enabirds/wav_Files.zip', '-d', 'benchmark/enabirds/'] & FG

local['wget']['https://storage.googleapis.com/ml-bioacoustics-datasets/enabirds_annotation.zip', '-O', 'benchmark/enabirds/annotation_Files.zip'] & FG
local['unzip']['benchmark/enabirds/annotation_Files.zip', '-d', 'benchmark/enabirds/'] & FG
...
for wav_path in sorted(Path('benchmark/enabirds/').glob('Recording_?/*.wav')):
    print(f'Converting {wav_path} ...', file=sys.stderr)

    target_paths = divide_waveform_to_chunks(
        path=wav_path,
        target_dir='benchmark/enabirds/wav',
...
for split in ['train', 'train-low', 'valid', 'test']:
    with open(f'benchmark/enabirds/{split}.jsonl', mode='w') as f:
