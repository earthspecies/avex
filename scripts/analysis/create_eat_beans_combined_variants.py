...
def load_df(path: Path) -> pd.DataFrame:
    ...
    -------
    pd.DataFrame
        Dataframe with models as rows and metric columns.
    """
...
def improvement_series(df: pd.DataFrame, general: str, bio: str) -> pd.Series:
    ...
    -------
    pd.Series
        Improvement values for each metric.
    """
...
def compute_benchmark_win_rates(
    ...
    """Compute win-rates by benchmark group using appropriate metrics.

    Returns
    -------
    dict[str, tuple[int, int, float]]
        Mapping of benchmark name to (wins, total, average_improvement).
    """
    if general not in df.index or bio not in df.index:
        return {}
...
def compute_benchmark_averages(df: pd.DataFrame) -> pd.DataFrame:
    ...
    -------
    pd.DataFrame
        Dataframe with benchmark-level averaged metrics.
    """
...
def create_variant1_plot(df: pd.DataFrame, output_dir: Path) -> None:
    """Create variant 1 plots for benchmark win-rates and BEANs scatter.
...
def create_variant2_plot(df: pd.DataFrame, output_dir: Path) -> None:
    """Create variant 2 plots for SSL and supervised win-rates plus scatter.
