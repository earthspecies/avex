...
def compute_benchmark_win_rates(df: pd.DataFrame, general: str, bio: str) -> Dict[str, Tuple[int, int, float]]:
...
def compute_benchmark_averages(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for group_name, datasets in benchmark_groups.items():
        for metric in ["R-auc", "Probe"]:  # Core metrics
            keys = [
                f"{ds}_{metric}" for ds in datasets 
...
def create_variant1_plot(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    bars = ax1.bar(display_groups, win_rates, color=['#00738B', '#04CDA0', '#C6DEE7', '#1ADCCF', '#98C6D2'], 
    ...
    ax1.set_title('Benefit of mixing general audio\nin pretraining', fontweight='bold', pad=15, fontsize=13)
    ax1.set_ylim(0, 105)
    ...
    new_marker = 'X'       # Cross
    group_properties = {
        'New Supervised Models': {'color': '#1ADCCF', 'marker': new_marker, 'label': 'New SL'},
        'Existing Supervised Models': {'color': '#1ADCCF', 'marker': existing_marker, 'label': 'Existing SL'},
        'New SSL Models': {'color': '#00738B', 'marker': new_marker, 'label': 'New SSL'},
        'Existing SSL Models': {'color': '#00738B', 'marker': existing_marker, 'label': 'Existing SSL'},
        'Post-trained SSL Models': {'color': '#F5E0B7', 'marker': new_marker, 'label': 'Post-trained SSL'}
    }
    ...
    for group_name, props in group_properties.items():
        ...
        if not group_data.empty:
            ax2.scatter(group_data[x_metric], group_data[y_metric], 
                      label=props['label'], alpha=1.0, s=100, 
    ...
    for idx, row in plot_data.iterrows():
        if not pd.isna(row[x_metric]) and not pd.isna(row[y_metric]):
            ...
            if "Bird-AVES-biox-base" in label:
                label = "Bird-AVES"
            ax2.annotate(label, (row[x_metric], row[y_metric]), 
    ...
    ax2.set_title("Bioacoustic generalization separates\nby training paradigm", 
                 pad=15, fontweight='bold', fontsize=13)
    ax2.grid(True, alpha=0.25)
    
    # Create factorial legend: Blue/Red = SSL/SL, Circle/X = Existing/New
    legend = ax2.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=10, 
                       title='Training Approach', title_fontsize=11)
...
def create_variant2_plot(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    bars = ax1.bar(display_labels, win_rates, color=colors, alpha=1.0, edgecolor='black', linewidth=1)
...
def create_variant3_plot(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    bars = ax1.bar(display_labels, win_rates, color=colors, alpha=0.8, edgecolor='black', linewidth=1)
    ...
    group_properties = {
        'New Supervised Models': {'color': sl_color, 'marker': new_marker, 'label': 'New SL'},
        'Existing Supervised Models': {'color': sl_color, 'marker': existing_marker, 'label': 'Existing SL'},
        'New SSL Models': {'color': ssl_color, 'marker': new_marker, 'label': 'New SSL'},
        'Existing SSL Models': {'color': ssl_color, 'marker': existing_marker, 'label': 'Existing SSL'},
        'Post-trained SSL Models': {'color': '#F5E0B7', 'marker': new_marker, 'label': 'Post-trained SSL'}
    }
    ...
    for group_name, props in group_properties.items():
        ...
        if not group_data.empty:
            ax2.scatter(group_data[x_metric], group_data[y_metric], 
                      label=props['label'], alpha=0.8, s=100, 
                      edgecolors='k', linewidths=0.5,
                      color=props['color'], marker=props['marker'])
            plotted_groups.append(group_name)
    ...
    ax2.set_ylabel("Retrieval BEANS Detection R-AUC", fontweight='bold', fontsize=12)
    ax2.set_title("Performance on CBI vs BEANS Detection\nseparates by training paradigm", 
                 pad=15, fontweight='bold', fontsize=13)
    ax2.grid(True, alpha=0.25)
...
def main() -> None:
    ...
    print("🔬 Creating EAT + BEANS Combined Analysis Variants (three variants: retrieval aggregates, retrieval aggregates, and CBI-specific)...")
