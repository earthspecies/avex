...
def compute_beans_averages_probe(df: pd.DataFrame, exclude_esc50: bool = False) -> pd.DataFrame:
    """Compute BEANS Classification and Detection aggregated averages using Probe metrics.
    ...
    -------
    pd.DataFrame
        Dataframe with computed BEANS averages using Probe metrics.
    """
    result = pd.DataFrame(index=df.index)
    ...
    # Compute Probe average for BEANS Classification
    metric = "Probe"
    keys = [f"{ds}_{metric}" for ds in beans_classification if f"{ds}_{metric}" in df.columns]
    if keys:
        values = df[keys].apply(pd.to_numeric, errors="coerce")
        avg = values.mean(axis=1, skipna=True)
        result["BEANS Classification_Probe"] = avg

    # Compute Probe average for BEANS Detection
    keys = [f"{ds}_{metric}" for ds in beans_detection if f"{ds}_{metric}" in df.columns]
    if keys:
        values = df[keys].apply(pd.to_numeric, errors="coerce")
        avg = values.mean(axis=1, skipna=True)
        result["BEANS Detection_Probe"] = avg

    return result


def compute_birdset_averages_probe(df: pd.DataFrame) -> pd.DataFrame:
    """Compute BirdSet aggregated averages using Probe metrics.

    Returns
    -------
    pd.DataFrame
        Dataframe with computed BirdSet averages using Probe metrics.
    """
    result = pd.DataFrame(index=df.index)
    ...
    # Compute Probe average for BirdSet
    metric = "Probe"
    keys = [f"{ds}_{metric}" for ds in birdset_datasets if f"{ds}_{metric}" in df.columns]
    if keys:
        values = df[keys].apply(pd.to_numeric, errors="coerce")
        avg = values.mean(axis=1, skipna=True)
        result["BirdSet_Probe"] = avg
...
def create_cbi_birdset_scatter_plot_probe(df: pd.DataFrame, output_dir: Path) -> None:
    """Create CBI vs BirdSet aggregated results scatter plot using Probe metrics.

    Similar to the CBI vs BEANS Detection plot but with BirdSet on Y-axis, using Probe metrics.
    """
    # Compute BirdSet aggregated results using Probe
    birdset_averages = compute_birdset_averages_probe(df)
    ...
    # Metrics to plot - CBI dataset specifically vs BirdSet aggregated (using Probe)
    x_metric = "CBI_Probe"  # CBI dataset Probe metric specifically
    y_metric = "BirdSet_Probe"  # Aggregated BirdSet Probe metric

    # Check if metrics exist - if CBI_Probe doesn't exist, try to get it from raw data
    if x_metric not in birdset_data.columns:
        # Try to get CBI directly from the original dataframe
        if "CBI_Probe" in df.columns:
            # Add CBI data to birdset_data
            for model in available_models:
                if model in df.index:
                    birdset_data.loc[model, x_metric] = df.loc[model, "CBI_Probe"]
        else:
            print(f"CBI Probe dataset not found. Available columns: {list(df.columns)[:20]}...")
            return

    if y_metric not in birdset_data.columns:
        print(f"Missing BirdSet Probe metric. Available: {list(birdset_data.columns)}")
        return
    ...
    if len(plot_data) == 0:
        print("No valid CBI vs BirdSet Probe data for plotting")
        return
    ...
    ax.set_xlabel("CBI Dataset Probe (Accuracy/mAP)", fontweight="bold", fontsize=12)
    ax.set_ylabel("Probe BirdSet (Accuracy/mAP)", fontweight="bold", fontsize=12)
    ax.set_title(
        "Performance on CBI vs BirdSet (Probe)\nseparates by training paradigm",
        pad=15,
        fontweight="bold",
        fontsize=13,
    )
    ...
    # Save the plot
    filename = "cbi_birdset_scatter_probe.png"
    filepath = output_dir / filename
    plt.savefig(filepath, dpi=300, bbox_inches="tight")
    plt.close()

    print(f"✅ Created CBI vs BirdSet scatter plot (Probe): {filename}")
...
def create_beans_detection_classification_scatter_plot_probe(
    df: pd.DataFrame, output_dir: Path, exclude_esc50: bool = False
) -> None:
    """Create BEANS Detection vs BEANS Classification scatter plot using Probe metrics.
    ...
    exclude_esc50 : bool, default False
        If True, exclude ESC-50 from BEANS Classification aggregation.
    """
    # Compute BEANS averages using Probe metrics
    beans_averages = compute_beans_averages_probe(df, exclude_esc50=exclude_esc50)
    ...
    # Metrics to plot using Probe
    x_metric = "BEANS Classification_Probe"  # BEANS Classification aggregated Probe
    y_metric = "BEANS Detection_Probe"  # BEANS Detection aggregated Probe

    if x_metric not in beans_data.columns:
        print(f"Missing BEANS Classification Probe metric. Available: {list(beans_data.columns)}")
        return

    if y_metric not in beans_data.columns:
        print(f"Missing BEANS Detection Probe metric. Available: {list(beans_data.columns)}")
        return
    ...
    if len(plot_data) == 0:
        print("No valid BEANS Detection vs BEANS Classification Probe data for plotting")
        return
    ...
    esc50_text = " (w/o ESC-50)" if exclude_esc50 else ""
    ax.set_xlabel(
        f"Probe BEANS Classification (Accuracy/mAP){esc50_text}", fontweight="bold", fontsize=12
    )
    ax.set_ylabel("Probe BEANS Detection (Accuracy/mAP)", fontweight="bold", fontsize=12)
    ax.set_title(
        f"Performance on BEANS Classification{esc50_text}"
        + "vs BEANS Detection (Probe)\nseparates by training paradigm",
        pad=15,
        fontweight="bold",
        fontsize=13,
    )
    ...
    # Save the plot
    suffix = "_noesc50" if exclude_esc50 else ""
    filename = f"beans_detection_classification_scatter_probe{suffix}.png"
    filepath = output_dir / filename
    plt.savefig(filepath, dpi=300, bbox_inches="tight")
    plt.close()

    print(f"✅ Created BEANS Detection vs BEANS Classification scatter plot (Probe): {filename}")
...
def main() -> None:
    ...
    # Create probe versions of the scatter plots
    # Create the CBI vs BirdSet scatter plot (Probe)
    create_cbi_birdset_scatter_plot_probe(df, out_dir)

    # Create the BEANS Detection vs BEANS Classification scatter plot (Probe, with ESC-50)
    create_beans_detection_classification_scatter_plot_probe(df, out_dir, exclude_esc50=False)

    # Create the BEANS Detection vs BEANS Classification scatter plot (Probe, without ESC-50)
    create_beans_detection_classification_scatter_plot_probe(df, out_dir, exclude_esc50=True)
