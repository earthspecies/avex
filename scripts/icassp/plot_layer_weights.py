...
def plot_combined_layer_weights(
    ...
    for _, base_model in enumerate(birdset_models):
        model_data = birdset_stats[birdset_stats["base_model"] == base_model].sort_values(
            "layer_index"
        )
        if len(model_data) == 0:
            continue
...
def plot_averaged_layer_weights(
    ...
    print(
        f"Across {combined_data['dataset'].nunique()} datasets: {combined_data['dataset'].unique()}"
    )
    ...
    combined_data_display["base_model_clean"] = combined_data_display["base_model"].apply(
        extract_base_model_name
    )
    combined_data_display["display_name"] = combined_data_display["base_model_clean"].map(
        model_name_mapping
    )
    ...
    combined_data_display["ssl_sl_class"] = combined_data_display["base_model_clean"].apply(
...
def plot_dataset_specific_heatmap(
    ...
    summary = combined_data.groupby(["subdataset", "layer_index", "ssl_sl"], as_index=False)[
        "weight"
    ].mean()
...
def plot_ssl_sl_heatmap(beans_csv: str, birdset_csv: str, output_path: str) -> None:
    ...
    # Filter out rows with missing layer weights
    combined_data = combined_data[
        combined_data["layer_weights"].notna() & (combined_data["layer_weights"] != "nan")
    ]
...
def main() -> None:
    ...
    if len(birdset_data) > 0:
        ...
        print(
            f"Birdset: Found {len(birdset_stats['base_model'].unique())} models with layer weights"
        )
    else:
