...
local['mkdir']['-p', 'benchmark/hainan_gibbons/wav']()
local['mkdir']['-p', 'benchmark/hainan_gibbons/Train']()
local['wget']['https://zenodo.org/record/3991714/files/Train.zip?download=1', '-O', 'benchmark/hainan_gibbons/Train.zip'] & FG
local['unzip']['benchmark/hainan_gibbons/Train.zip', '-d', 'benchmark/hainan_gibbons/Train/'] & FG
local['wget']['https://zenodo.org/record/3991714/files/Train_Labels.zip?download=1', '-O', 'benchmark/hainan_gibbons/Train_Labels.zip'] & FG
local['unzip']['benchmark/hainan_gibbons/Train_Labels.zip', '-d', 'benchmark/hainan_gibbons/'] & FG
...
for file_id, wav_path in enumerate(sorted(Path('benchmark/hainan_gibbons/').glob('Train/*.wav'))):
    print(f'Converting {wav_path} ...', file=sys.stderr)

    target_paths = divide_waveform_to_chunks(
        path=wav_path,
        target_dir='benchmark/hainan_gibbons/wav',
...
for split in ['train', 'train-low', 'valid', 'test']:
    with open(f'benchmark/hainan_gibbons/{split}.jsonl', mode='w') as f:
