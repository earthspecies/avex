...
local['git']['clone', 'https://github.com/HumBug-Mosquito/HumBugDB.git', 'benchmark/HumBugDB'] & FG

for i in [1, 2, 3, 4]:
    local['wget']['-O', f'benchmark/HumBugDB/humbugdb_neurips_2021_{i}.zip',
                  f'https://zenodo.org/record/4904800/files/humbugdb_neurips_2021_{i}.zip?download=1'] & FG
    local['unzip'][f'benchmark/HumBugDB/humbugdb_neurips_2021_{i}.zip', '-d', 'benchmark/HumBugDB/data/audio/'] & FG


df = pd.read_csv('benchmark/HumBugDB/data/metadata/neurips_2021_zenodo_0_0_1.csv')
...
def convert(row):
    new_row = pd.Series({
        'path': f"benchmark/HumBugDB/data/audio/{row['id']}.wav",
        'label': row['species']
    })
...
df_train.to_csv('benchmark/HumBugDB/data/metadata/train.csv')
df_train_low.to_csv('benchmark/HumBugDB/data/metadata/train-low.csv')
df_valid.to_csv('benchmark/HumBugDB/data/metadata/valid.csv')
df_test.to_csv('benchmark/HumBugDB/data/metadata/test.csv')
