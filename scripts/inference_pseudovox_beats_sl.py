...
label_to_idx = pd.read_json('sl_beats_animalspeak_label_map.json', typ='series').to_dict()
...
class PseudovoxDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return df_idx, waveform  # return df index so we can write back later
...
for col in [
    ...
    "custom_classifier_max_logit",
]:
    if col not in shard_metadata.columns:
        shard_metadata[col] = None

with torch.no_grad():
    for df_indices, batch_waveforms in tqdm(loader):
        ...
        for df_idx, logit_vec in zip(df_indices, logits_np):
            df_idx = int(df_idx)
            for key in ["species_scientific_gbif", "species_scientific_animalspeak"]:
                ...
                species_in_metadata = shard_metadata.loc[df_idx, key]
            ...
            shard_metadata.at[df_idx, "custom_classifier_max_logit"] = max(
                shard_metadata.at[
                    df_idx, "custom_classifier_logit_for_species_scientific_gbif"
                ],
                shard_metadata.at[
                    df_idx,
                    "custom_classifier_logit_for_species_scientific_animalspeak",
...
# -----------------------
out_csv = (
    "animalspeak_pseudovox_with_birdnet_with_qf_with_c_with_meta_beatssl"
    f"_shard{SHARD}.csv"
)
